...
app.use("/sofficefiles", express.static(path.join(__dirname, "sofficefiles"))); // Serve static files
 ...
 */
 app.post("/upload", upload.single("file"), async (req, res) => {
    ...
    const { conversionOption } = req.body; // 'pandoc' or 'libreoffice'
    ...
    try {
        if (conversionOption === "libreoffice") {
            // Use LibreOffice conversion (soffice)
            //const wordFilePath = req.file.path;
            const command = `soffice --headless --convert-to html:HTML --outdir sofficefiles "${inputFilePath}"`;
            ...
            exec(command, (error, stdout, stderr) => {
                ...
                const convertedFileName = path.basename(inputFilePath, path.extname(inputFilePath)) + ".html";
                ...
                fs.readFile(convertedFilePath, "utf8", (err, htmlContent) => {
                    ...
                    const sofficebaseUrl = "http://localhost:5000/sofficefiles"; // Ensure this matches your backend

                    // Ensure all images have the base URL added
                    const updatedHtml = htmlContent.replace(
                        /src="([^"]+\.(gif|png|jpg|jpeg))"/g, 
                        `src="${sofficebaseUrl}/$1"`
                    );

                    res.json({ html: updatedHtml, html_file: `http://localhost:5000/sofficefiles/${convertedFileName}` });
            ...
            await new Promise((resolve, reject) => {
                exec(
                    `soffice --headless --convert-to html --outdir "${uniqueFolder}" "${inputFilePath}"`,
                    (error) => {
                        if (error) {
            ...
            */
            console.log("✅ Conversion completed with LibreOffice");
        } else if (conversionOption === "pandoc") {
            ...
            console.log("✅ Conversion completed with Pandoc");
